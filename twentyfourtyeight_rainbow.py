...
def draw_grid(score, lost, present, future, rate):
	...
	for row in range(grid_height):
		for col in range(grid_height):
			...
			if 0 < grid[row][col] <= 17:
				if future <= 2:
					if future == 1:
						pygame.draw.rect(screen, (color, round(color*present/rate-present/rate), 0), rect)
					else:
						pygame.draw.rect(screen, (round(color*(rate-present)/rate-(rate-present)/rate), color, 0), rect)
				elif future <= 4:
					if future == 3:
						pygame.draw.rect(screen, (0, color, round(color*present/rate-(present/rate))), rect)
					else:
						pygame.draw.rect(screen, (0, round(color*(rate-present)/rate-((rate-present)/rate)), color), rect)
				else:
					if future == 5:
						pygame.draw.rect(screen, (round(color*(present/rate)-(present/rate)), 0, color), rect)
					else:
						pygame.draw.rect(screen, (color, 0, round(color*(rate-present)/rate-((rate-present)/rate))), rect)
			...
			if grid[row][col] != 0:
				...
				if grid[row][col] <= 8:
					...
					if grid[row][col] <= 20:
						...
						text = font.render('2^'+str(grid[row][col]), True, (0, 0, 0))
...
def main():
	...
	while run:
		...
		if future <= 2:
			if future == 1:
				screen.fill((255, round(255*present/rate-present/rate), 0))
			else:
				screen.fill((round(255*(rate-present)/rate-(rate-present)/rate), 255, 0))
		elif future <= 4:
			if future == 3:
				screen.fill((0, 255, round(255*present/rate-(present/rate))))
			else:
				screen.fill((0, round(255*(rate-present)/rate-((rate-present)/rate)), 255))
		else:
			if future == 5:
				screen.fill((round(255*(present/rate)-(present/rate)), 0, 255))
			else:
				screen.fill((255, 0, round(255*(rate-present)/rate-((rate-present)/rate))))
